...
class Student(Base):
    ...
    features = relationship(
        "StudentFeature", back_populates="student", cascade="all, delete-orphan"
    )
    ...
    discipline_incidents = relationship(
        "DisciplineIncident", back_populates="student", cascade="all, delete-orphan"
    )
...
class Prediction(Base):
    ...
    risk_category = Column(
        String(20), CheckConstraint("risk_category IN ('low', 'medium', 'high', 'critical')")
    )
    ...
    feedback = relationship(
        "TrainingFeedback", back_populates="prediction", cascade="all, delete-orphan"
    )
