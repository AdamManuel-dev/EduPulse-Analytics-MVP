...
async def update_model(request: schemas.TrainingUpdateRequest, db: Session = Depends(get_db)):
    """
    ...
    return schemas.TrainingUpdateResponse(
        update_id=update_id, status="queued", estimated_completion=estimated_completion
    )


@router.get("/status/{update_id}")
async def get_update_status(update_id: str, db: Session = Depends(get_db)):
    """
