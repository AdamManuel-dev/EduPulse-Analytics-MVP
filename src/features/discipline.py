...
class DisciplineFeatureExtractor(BaseFeatureExtractor):
    ...
    def extract(self, student_id: str, reference_date: date) -> Dict[str, float]:
        ...
        incidents = (
            self.db.query(models.DisciplineIncident)
            .filter(
                and_(
            ...
            .order_by(models.DisciplineIncident.incident_date)
            .all()
        )
        ...
        recent_severity = (
            np.mean([i.severity_level for i in recent_incidents if i.severity_level is not None])
            if recent_incidents
            else 0.0
        )
