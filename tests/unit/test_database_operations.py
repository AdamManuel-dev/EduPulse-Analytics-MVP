...
class TestStudentModel:
    ...
    def test_student_relationships(self, db_session):
        ...
        attendance1 = models.AttendanceRecord(
            student_id=student.id, date=date.today(), status="present"
        )
        attendance2 = models.AttendanceRecord(
            student_id=student.id, date=date.today() - timedelta(days=1), status="absent"
        )
    ...
    def test_student_query_operations(self, db_session, batch_students):
        ...
        found_student = (
            db_session.query(models.Student).filter(models.Student.id == student.id).first()
        )
        ...
        found_by_district = (
            db_session.query(models.Student)
            .filter(models.Student.district_id == student.district_id)
            .first()
        )
        ...
        grade_10_students = (
            db_session.query(models.Student).filter(models.Student.grade_level == 10).all()
        )
        ...
        # Test ordering
        ordered_students = (
            db_session.query(models.Student).order_by(models.Student.gpa.desc()).limit(3).all()
        )
...
class TestAttendanceModel:
    ...
    def test_attendance_status_values(self, db_session, sample_student):
        ...
        for i, status in enumerate(valid_statuses):
            attendance = models.AttendanceRecord(
                student_id=sample_student.id, date=date.today() - timedelta(days=i), status=status
            )
            db_session.add(attendance)
        ...
        records = (
            db_session.query(models.AttendanceRecord)
            .filter(models.AttendanceRecord.student_id == sample_student.id)
            .all()
        )
    ...
    def test_attendance_date_queries(self, db_session, sample_student):
        ...
        for i in range(31):  # 31 days
            ...
            attendance = models.AttendanceRecord(
                student_id=sample_student.id, date=record_date, status=status
            )
            db_session.add(attendance)
...
class TestGradeModel:
    ...
    def test_grade_calculations(self, db_session, sample_student):
        ...
        student_grades = (
            ...
            .filter(models.Grade.student_id == sample_student.id)
            .all()
        )
    ...
    def test_grade_course_filtering(self, db_session, sample_student):
        ...
        math_grades = (
            db_session.query(models.Grade)
            .filter(
                models.Grade.student_id == sample_student.id, models.Grade.course_id == "MATH101"
            )
            .all()
        )
...
class TestDisciplineModel:
    ...
    def test_severity_level_queries(self, db_session, sample_student):
        ...
        severe_incidents = (
            db_session.query(models.DisciplineIncident)
            .filter(
    ...
    def test_incident_chronology(self, db_session, sample_student):
        ...
        chronological_incidents = (
            db_session.query(models.DisciplineIncident)
            .filter(models.DisciplineIncident.student_id == sample_student.id)
            .order_by(models.DisciplineIncident.incident_date.asc())
            .all()
        )
        ...
        for i in range(len(chronological_incidents) - 1):
            assert (
                chronological_incidents[i].incident_date
                <= chronological_incidents[i + 1].incident_date
            )
...
class TestPredictionModel:
    ...
    def test_prediction_queries(self, db_session, sample_student):
        ...
        latest_prediction = (
            ...
            .filter(models.Prediction.student_id == sample_student.id)
            .order_by(models.Prediction.prediction_date.desc())
            .first()
        )
...
class TestDatabaseSessionOperations:
    ...
    def test_transaction_rollback(self, db_session):
        ...
        attendance = models.AttendanceRecord(
            student_id=student_id, date=date.today(), status="present"
        )
        db_session.add(attendance)
        ...
        found_student = (
            db_session.query(models.Student)
            .filter(models.Student.district_id == "ROLLBACK_TEST")
            .first()
        )
    ...
    def test_bulk_operations(self, db_session):
        ...
        # Verify bulk insert
        bulk_students = (
            db_session.query(models.Student).filter(models.Student.district_id.like("BULK_%")).all()
        )

        assert len(bulk_students) == 10

        # Test bulk update
        db_session.query(models.Student).filter(models.Student.district_id.like("BULK_%")).update(
            {models.Student.gpa: 3.5}
        )
    ...
    def test_database_constraints_and_indexes(self, db_session):
        ...
        attendance = models.AttendanceRecord(
            student_id=fake_student_id, date=date.today(), status="present"
        )
        db_session.add(attendance)
