...
class TestFeaturePipeline:
    ...
    def mock_extractors(self):
        ...
        attendance_extractor.extract.return_value = {
        ...
        grade_extractor.extract.return_value = {
            'grade_mean': 82.5,
            'gpa': 3.2
        }
        ...
        discipline_extractor.extract.return_value = {
            'incident_count': 1.0,
            'severity_max': 2.0
        }
    ...
    def test_initialization_with_cache(self, mock_db_session):
        ...
        with patch('src.features.pipeline.redis.Redis') as mock_redis_class, \
             patch('src.features.pipeline.AttendanceFeatureExtractor') as mock_att, \
             patch('src.features.pipeline.GradeFeatureExtractor') as mock_grade, \
             patch('src.features.pipeline.DisciplineFeatureExtractor') as mock_disc:
    ...
    def test_initialization_without_cache(self, mock_db_session):
        ...
        with patch('src.features.pipeline.AttendanceFeatureExtractor') as mock_att, \
             patch('src.features.pipeline.GradeFeatureExtractor') as mock_grade, \
             patch('src.features.pipeline.DisciplineFeatureExtractor') as mock_disc:
    ...
    def test_extract_features_no_cache(self, mock_db_session, mock_extractors):
        ...
        with patch('src.features.pipeline.AttendanceFeatureExtractor') as mock_att, \
            ...
            expected_features = ['attendance_rate', 'absence_streak', 'grade_mean', 'gpa', 'incident_count', 'severity_max']
            for feature_name in expected_features:
                assert feature_name in features
            
            # Verify extractors were called
            mock_extractors['attendance'].extract.assert_called_once_with(student_id, reference_date)
            mock_extractors['grades'].extract.assert_called_once_with(student_id, reference_date)
            mock_extractors['discipline'].extract.assert_called_once_with(student_id, reference_date)
    ...
    def test_get_feature_names(self, mock_db_session, mock_extractors):
        ...
        with patch('src.features.pipeline.AttendanceFeatureExtractor') as mock_att, \
            ...
            expected_names = [
                'attendance_rate', 'absence_streak',
                'grade_mean', 'gpa',
                'incident_count', 'severity_max'
            ]
    ...
    def test_get_cache_key(self, mock_db_session):
        ...
        with patch('src.features.pipeline.AttendanceFeatureExtractor'), \
             patch('src.features.pipeline.GradeFeatureExtractor'), \
