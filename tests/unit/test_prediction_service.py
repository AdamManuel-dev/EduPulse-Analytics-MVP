...
class TestPredictionService:
    ...
    def test_prediction_service_initialization(self):
        ...
        with patch('src.services.prediction_service.GRUAttentionModel'), \
             patch('src.services.prediction_service.FeaturePipeline'):
            service = PredictionService()
            assert service.model is not None or service.model is None  # May be None if model loading fails
    ...
    def test_model_device_handling(self, prediction_service):
        ...
        with patch.object(prediction_service, 'prepare_sequence') as mock_prep:
            ...
            prediction_service.model.forward = Mock(return_value=(
                torch.tensor([[0.5]]), torch.zeros(1, 1, 5)
            ))
