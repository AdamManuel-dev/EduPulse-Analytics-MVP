...
from src.training.trainer import StudentSequenceDataset, ModelTrainer
...
class TestModelTrainer:
    ...
    def test_trainer_initialization_uses_settings(self, mock_get_settings, mock_model):
        ...
        mock_settings.model_early_stopping_patience = 15
        ...
        assert trainer.early_stopping.patience == 15

    def test_train_epoch(self, mock_model):
        """Test training epoch execution."""
        trainer = ModelTrainer(model=mock_model)
        
        # Mock model output - ensure risk predictions are properly shaped
        mock_model.return_value = (
            torch.sigmoid(torch.randn(1, 1)),  # Correct shape for BCE loss
            torch.randn(1, 4),  # category logits
            torch.randn(1, 10, 42)  # attention weights
        )
        
        # Create properly shaped batch data
        X = torch.randn(1, 10, 42)
        y_risk = torch.tensor([[0.5]])  # Shape should match model output
        y_category = torch.tensor([1])   # Shape for CrossEntropy
        
        dataloader = DataLoader([(X, y_risk, y_category)], batch_size=1)
    ...
    def test_load_model(self, mock_model, tmp_path):
        ...
        with patch('torch.load', return_value=checkpoint) as mock_torch_load:
            ...
            trainer.optimizer.load_state_dict.assert_called_once_with(checkpoint['optimizer_state_dict'])
